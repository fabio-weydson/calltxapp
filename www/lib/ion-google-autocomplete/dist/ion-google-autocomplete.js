...
.directive('googleAutocompleteSuggestion', [
    ...
    return {
        ...
        link: function($scope, element) {
            ...
            var template = [
                '<ion-modal-view id="modal_sugest">',
                '<ion-header-bar class="item-input-inset">',
                '<label class="item-input-wrapper">',
                '<i class="icon ion-search placeholder-icon"></i>',
                '<input type="search" ng-model="search.query" placeholder="Comece a digitar...">',
                '</label>',
                '<button class="ion-autocomplete-cancel button button-clear button-dark ng-binding" ng-click="close()" translate>Ok</button>',
                '</ion-header-bar>',
                '<ion-content>',
                '<ion-list>',
            ...
            $scope.choosePlace = function(place) {
                ...
                googleAutocompleteService.getDetails(place.place_id).then(function(location) {
                    //console.log(location.name, location.formatted_address)
                    $scope.location = location;
                    $scope.location['place_id'] = location.place_id;
                    $scope.location['LatLng'] = location.geometry.location.lat()+','+location.geometry.location.lng();
                    $scope.close();
